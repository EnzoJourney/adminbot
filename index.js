...
client.on("ready", function() {
    client.user.setGame("?help | Yt: Enzo Journey");
    console.log("Le bot a bien été connecté");
})

const warns = JSON.parse(fs.readFileSync('./warns.json'))
 
client.login('NTYyNjQ4OTkyMjQ1MjE5MzU4.XKumPA.TM9CtACZ1M21MbSXrbedVZaJ_qQ');
 
client.on('message', message =>{
    if(message.content === "?help"){
        message.channel.sendMessage('Mes différentes commandes sont : ?kick (membre) / ?ban (membre) / ?clear (nombre de message) / ?mute (membre) / ?warn (pseudo) (raison) / ?infractions (membre) / ?8ball (question)');
        console.log('envoie la liste de mommandes');
    }
});
...
client.on('guildMemberAdd', member =>{
    ...
    member.guild.channels.get('440201409934393344').send(embed)
    member.addRole('428274764554043413')
...
client.on('guildMemberRemove', member =>{
    let embed = new Discord.RichEmbed()
        ...
        .setFooter('Nous sommes désormais ' + member.guild.memberCount)
    member.guild.channels.get('562960870490243083').send(embed)
